...
function jfb_admin_page()
{
    ...
    if( isset($_POST['main_opts_updated']) )
    {
        ...
        update_option( $opt_jfb_ask_link, esc_html( isset( $_POST[$opt_jfb_ask_link] ) ? $_POST[$opt_jfb_ask_link] : 0 ) );
    ...
    <div class="jfb-admin_wrapper">
		...
		<h3><?php _e("Setup Instructions", 'wp-fb-autoconnect')?></h3>
            ...
            <ol>
			  <li><?php echo sprintf(__("Visit %s and click 'Add a New App.'", 'wp-fb-autoconnect'), "<a href='http://developers.facebook.com/apps' target='lnk'>developers.facebook.com/apps</a>")?></li>
			  <li><?php _e("When the 'Create a New App ID' window pops up, type in a name (i.e. the name of your site) &amp; contact e-mail, and click 'Create App ID.'", 'wp-fb-autoconnect')?></li>
			  <li><?php _e("Go to Settings-&gt;Basic (link on the left), click 'Add Platform' (at the bottom), 'Website,' then fill in your 'Site URL.'", 'wp-fb-autoconnect')?><br/>
			  ...
			  <li><?php _e("Go to the 'App Review' page (link on the left) and make the app public (flip the switch at the top).", 'wp-fb-autoconnect')?></li>
        ...
        <div class="<?php echo $allTabsClass ?>" id="<?php echo $tabIds[1]?>" style="display:<?php echo ($shownTab==1?"block":"none")?>">
            ...
            <form name="formMainOptions" method="post" action="">
        		...
        		<?php jfb_output_simple_lightbox("", __("Until API v3.0, Facebook used to automatically give access to users' profile URLs. Unfortunately, this now requires special permission. For this option to work, you will need to first submit your app for review by Facebook: go to the app's settings, then 'App Review'->'Start a Submission,' &amp; ask for the 'user_link' permission. Once they approve you, users who login will have their profile URL fetched &amp; listed on their WP profile page (assuming they don't explicitly deny the permission when they login).", 'wp-fb-autoconnect'))?><br />
