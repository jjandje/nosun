/* global jQuery rmlOpts RMLFindDeep RMLFormat sweetAlert RMLUrlParams */

(function( $ ) {
    $(function() { // Mini javascript libraries
        if (!window.sweetAlert && RMLUrlParams().post_type !== "inbound-email") { // Inbound Now Pro compatibility, perhaps they update sweetalert because they are using a very old version...
...
jQuery(document).ready(function() {
    ...
    if (RMLFindDeep(window, "wp.media")) {
                ...
                onClick: function() {
                    ...
                    if (toDelete.parent().children("ul").children("li").length > 0) {
                        ...
                        sweetAlert({   
                            ...
                            text: RMLFormat(rmlOpts.lang.deleteConfirm, toDelete.find(".aio-node-name").attr("title")),   
                            type: "warning",   
                            showCancelButton: true,   
                            confirmButtonColor: "#DD6B55",   
        ...
        movement: {
            ...
            droppableSettings: {
                ...
                drop: function(event, ui) {
                    ...
                    // If the target is "Uncategorized" the current folder has to be refreshed
                    if (+folderId === -1) {
                        container.allInOneTree("active").addClass("needs-refresh");
                    }
                            ...
                            function(response) {
                                if (response.success) {
                                    ...
                                    removeElementsIterate(function() {
                                        ...
                                        // Remove, or not
                                        if (isShortcut || (!isShortcut && activeId == folderId) || activeId == "") {
                                            $(this).fadeTo(0, 1);
                                        }else{
                                            // Remove from view
                                            $(this).remove();
