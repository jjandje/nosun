 ...
 *
 */
class Nosun_Assumax_Api_Admin_Settings {
	...
	public function setup_plugin_options_menu() {
		...
		add_plugins_page( 'Assumax API settings',                    // The title to be displayed in the browser window for this page.
			'API settings',                    // The text to be displayed for this menu item
			'manage_options',                    // Which type of users can see this menu item
			'nosun_api_settings',            // The unique ID - that is, the slug - for this menu item
			array (
				$this,
	...
	public function render_settings_page_content( $active_tab = '' ) {
		...
		<div class="wrap">
			...
			if ( isset( $_GET[ 'tab' ] ) ) {
				...
				$active_tab = 'tab_import';
			} // end if/else ?>

			<h2 class="nav-tab-wrapper">
				<a href="?page=wppb_demo_options&tab=display_options" class="nav-tab <?php echo $active_tab == 'tab_import' ? 'nav-tab-active' : ''; ?>">Import</a>
				<!--
			...
			<form method="post" action="options.php">
				...
				if ( $active_tab == 'tab_import' ) {

					settings_fields( 'import_display_options' );
					do_settings_sections( 'import_display_options' );
	...
	public function general_options_callback() {
		$options = get_option( 'import_display_options' );
		//var_dump( $options );
		echo '<p></p>';
	...
	public function initialize_display_options() {
		...
		if ( false == get_option( 'import_display_options' ) ) {
			$default_array = $this->default_display_options();
			add_option( 'import_display_options', $default_array );
		}


		add_settings_section( 'general_settings_section',                        // ID used to identify this section and with which to register options
			'Import',                // Title to be displayed on the administration page
			array (
				$this,
				'general_options_callback'
			),        // Callback used to render the description of the section
			'import_display_options'                        // Page on which to add this section of options
		);

		// Next, we'll introduce the fields for toggling the visibility of content elements.
		add_settings_field( 'show_header',                                // ID used to identify the field throughout the theme
			__( 'Header', 'wppb-demo-plugin' ),                    // The label to the left of the option interface element
			array (
				$this,
				'toggle_header_callback'
			),    // The name of the function responsible for rendering the option interface
			'import_display_options',                // The page on which this option will be displayed
			'general_settings_section',                    // The name of the section to which this field belongs
			array (                                        // The array of arguments to pass to the callback. In this case, just a description.
				__( 'Activate this setting to display the header.', 'wppb-demo-plugin' ),
			) );

		add_settings_field( 'show_content', __( 'Content', 'wppb-demo-plugin' ), array (
				$this,
				'toggle_content_callback'
			), 'import_display_options', 'general_settings_section', array (
				__( 'Activate this setting to display the content.', 'wppb-demo-plugin' ),
			) );

		add_settings_field( 'show_footer', __( 'Footer', 'wppb-demo-plugin' ), array (
				$this,
		...
		register_setting( 'import_display_options', 'import_display_options' );
	...
	public function initialize_social_options() {
		...
		add_settings_section( 'social_settings_section',            // ID used to identify this section and with which to register options
		...
		add_settings_field( 'twitter', 'Twitter', array (
				$this,
				'twitter_callback'
			), 'wppb_demo_social_options', 'social_settings_section' );

		add_settings_field( 'facebook', 'Facebook', array (
				$this,
				'facebook_callback'
			), 'wppb_demo_social_options', 'social_settings_section' );

		add_settings_field( 'googleplus', 'Google+', array (
				$this,
				'googleplus_callback'
			), 'wppb_demo_social_options', 'social_settings_section' );

		register_setting( 'wppb_demo_social_options', 'wppb_demo_social_options', array (
				$this,
	...
	public function initialize_input_examples() {
		...
		add_settings_section( 'input_examples_section', __( 'Input Examples', 'wppb-demo-plugin' ), array (
				$this,
				'input_examples_callback'
			), 'wppb_demo_input_examples' );

		add_settings_field( 'Input Element', __( 'Input Element', 'wppb-demo-plugin' ), array (
				$this,
				'input_element_callback'
			), 'wppb_demo_input_examples', 'input_examples_section' );

		add_settings_field( 'Textarea Element', __( 'Textarea Element', 'wppb-demo-plugin' ), array (
				$this,
				'textarea_element_callback'
			), 'wppb_demo_input_examples', 'input_examples_section' );

		add_settings_field( 'Checkbox Element', __( 'Checkbox Element', 'wppb-demo-plugin' ), array (
				$this,
				'checkbox_element_callback'
			), 'wppb_demo_input_examples', 'input_examples_section' );

		add_settings_field( 'Radio Button Elements', __( 'Radio Button Elements', 'wppb-demo-plugin' ), array (
				$this,
				'radio_element_callback'
			), 'wppb_demo_input_examples', 'input_examples_section' );

		add_settings_field( 'Select Element', __( 'Select Element', 'wppb-demo-plugin' ), array (
				$this,
				'select_element_callback'
			), 'wppb_demo_input_examples', 'input_examples_section' );

		register_setting( 'wppb_demo_input_examples', 'wppb_demo_input_examples', array (
				$this,
	...
	public function toggle_header_callback( $args ) {
		...
		$html = '<input type="checkbox" id="show_header" name="import_display_options[show_header]" value="1" ' . checked( 1, isset( $options[ 'show_header' ] ) ? $options[ 'show_header' ] : 0, false ) . '/>';
	...
	public function toggle_content_callback( $args ) {
		...
		$html = '<input type="checkbox" id="show_content" name="import_display_options[show_content]" value="1" ' . checked( 1, isset( $options[ 'show_content' ] ) ? $options[ 'show_content' ] : 0, false ) . '/>';
	...
	public function toggle_footer_callback( $args ) {
		...
		$html = '<input type="checkbox" id="show_footer" name="import_display_options[show_footer]" value="1" ' . checked( 1, isset( $options[ 'show_footer' ] ) ? $options[ 'show_footer' ] : 0, false ) . '/>';
	...
	public function select_element_callback() {
		...
		$html .= '<option value="always"' . selected( $options[ 'time_options' ], 'always', false ) . '>' . __( 'Always', 'wppb-demo-plugin' ) . '</option>';
		$html .= '</select>';
