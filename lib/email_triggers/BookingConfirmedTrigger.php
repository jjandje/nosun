...
/**
 * Triggers when a Booking changes status to Confirmed.
 *
 * Class BookingConfirmedTrigger
 * @package lib\email_triggers
 */
class BookingConfirmedTrigger extends Trigger {
    /**
     * @inheritDoc
     */
    ...
    static function add_available_email_triggers(array $triggers): array {
        $triggers['booking_confirmed'] = new TriggerInformation(
            _x('Boeking bevestigd', 'email_triggers', 'vazquez'),
            _x('Triggert wanneer de status van een boeking naar Confirmed verandert. Eenmalig betekent hier dat er maar 1 e-mail gestuurd wordt per klant per boeking id.', 'email_triggers', 'vazquez'),
            [
...
add_filter('vazquez_email_available_triggers', [BookingConfirmedTrigger::class, 'add_available_email_triggers']);
