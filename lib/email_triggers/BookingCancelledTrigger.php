...
/**
 * Triggers when a Booking changes status to cancelled.
 *
 * Class BookingCancelledTrigger
 * @package lib\email_triggers
 */
class BookingCancelledTrigger extends Trigger {
    /**
     * @inheritDoc
     */
    ...
    static function add_available_email_triggers(array $triggers): array {
        $triggers['booking_cancelled'] = new TriggerInformation(
            _x('Boeking geannuleerd', 'email_triggers', 'vazquez'),
            _x('Triggert wanneer de status van een boeking naar Cancelled verandert. Eenmalig betekent hier dat er maar 1 e-mail gestuurd wordt per klant per boeking id.', 'email_triggers', 'vazquez'),
            [
...
add_filter('vazquez_email_available_triggers', [BookingCancelledTrigger::class, 'add_available_email_triggers']);
