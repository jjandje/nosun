...
/**
 * Triggers when a rest payment has been received.
 *
 * Class RestPaymentReceivedTrigger
 * @package lib\email_triggers
 */
class RestPaymentReceivedTrigger extends Trigger
{
    /**
     * @inheritDoc
     */
    ...
    static function add_available_email_triggers(array $triggers): array {
        $triggers['rest_payment_received'] = new TriggerInformation(
            _x('Restbetaling ontvangen', 'email_triggers', 'vazquez'),
            _x('Triggert wanneer er een restbetaling is ontvangen. Eenmalig betekent hier dat er maar 1 e-mail gestuurd wordt per boeking id en type betaling.', 'email_triggers', 'vazquez'),
            [
...
add_filter('vazquez_email_available_triggers', [RestPaymentReceivedTrigger::class, 'add_available_email_triggers']);
